...
class CCodeGenerator:
    ...
    def generate(self, output_dir: str):
        # Output files follow rpcgen naming: foo.x -> foo.h, foo.c.
        # The prefix is the .x basename (e.g. "nfsv42_xdr" for nfsv42_xdr.x).
        h_file = os.path.join(output_dir, self.prefix + '.h')
        c_file = os.path.join(output_dir, self.prefix + '.c')

        guard = self.prefix.upper() + '_H'
        ...
        # Forward-declare every named struct/union before any typedef or member
        # definition uses the type name.  This allows XDR files that contain
        # "typedef struct foo *bar" before "struct foo { ... }" to compile
        # without requiring the admin to reorder the source definitions.
        fwd = [d for d in self.spec.definitions if isinstance(d, (StructDef, UnionDef))]
        if fwd:
            for defn in fwd:
                h_lines.append("typedef struct %s %s;\n" % (defn.name, defn.name))
            h_lines.append("\n")

        # C implementation file
        c_lines.append("/* DO NOT EDIT - generated by xdr_parser.py from %s.x */\n" % self.prefix)
        c_lines.append("/* Apply changes to %s.x instead. */\n\n" % self.prefix)
        c_lines.append('#include "%s.h"\n\n' % self.prefix)
    ...
    def _gen_c_defn(self, defn):
        if isinstance(defn, ConstDef):
            ...
            c += "\tif (!xdr_enum(xdrs, (enum_t *)objp))\n\t\treturn FALSE;\n"
            c += "\treturn TRUE;\n}\n\n"
            return h, c

        elif isinstance(defn, StructDef):
            # typedef struct foo foo; was already emitted in the forward-decl
            # first pass in generate().  Just emit the struct body here.
            h = "struct %s {\n" % defn.name
            for d in defn.body:
            ...
            for cs in defn.cases:
                ...
                if isinstance(cs.declaration, VoidDecl):
                    ...
                    # _c_xdr_member returns code at \t indent; add one more tab for
                    # the switch case body (\t\t for the if, \t\t\t for return FALSE).
                    arm = self._c_xdr_member(cs.declaration,
                                             'objp->%s' % union_u, sep='.')
                    c += self._case_indent(arm)
                    c += "\t\tbreak;\n"
            if defn.default_arm is not None:
                c += "\tdefault:\n"
                if isinstance(defn.default_arm, VoidDecl):
                    ...
                    arm = self._c_xdr_member(defn.default_arm,
                                             'objp->%s' % union_u, sep='.')
                    c += self._case_indent(arm)
                    c += "\t\tbreak;\n"
            else:
                # No explicit default in XDR spec. Add an implicit default to
                # suppress -Werror,-Wswitch when the discriminant enum has values
                # not covered by any case arm.
                c += "\tdefault:\n\t\tbreak;\n"
            c += "\t}\n"
            c += "\treturn TRUE;\n}\n\n"
            return h, c
    ...
    def _case_indent(self, code: str) -> str:
        """Add one tab level to code generated by _c_xdr_member for use inside
        a switch case body.  _c_xdr_member returns code at \\t indent; a switch
        case body needs \\t\\t for statements and \\t\\t\\t for continuations."""
        # Each line already starts with \\t; prepend one more tab.
        return '\t' + code.replace('\n\t', '\n\t\t').rstrip('\t')
    ...
    def _c_xdr_member(self, decl: Declaration, obj: str, sep: str = '->') -> str:
        ...
        if isinstance(decl, SimpleDecl):
            ...
            if xdr_func:
                return "\tif (!%s(xdrs, &%s))\n\t\treturn FALSE;\n" % (xdr_func, accessor)
            # Fixed-array typedefs (OpaqueFixedDecl, FixedArrayDecl) take the
            # array by value (decays to pointer) -- do not add '&'.
            resolved = self.name_dict.get(type_name)
            if isinstance(resolved, TypeDef) and isinstance(
                    resolved.declaration, (OpaqueFixedDecl, FixedArrayDecl)):
